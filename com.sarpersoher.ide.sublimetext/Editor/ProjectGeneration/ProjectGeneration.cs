...
";
        ...
        void CreateSublimeTextProject() {
            var sublimeTextProjectFile = Path.Combine(ProjectDirectory, $"{m_ProjectName}.sublime-project");

            if(!m_FileIOProvider.Exists(sublimeTextProjectFile))
                m_FileIOProvider.WriteAllText(sublimeTextProjectFile, k_SettingsJson);
        }
    }
